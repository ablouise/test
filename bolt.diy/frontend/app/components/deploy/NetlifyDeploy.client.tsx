...
export function useNetlifyDeploy() {
  ...
  const handleNetlifyDeploy = async () => {
    ...
    try {
      ...
      /*
       *  Get the build files Todo: Louise!
       * const container = await dockerClient;
       */

      // Remove /home/project from buildPath if it exists
      const buildPath = artifact.runner.buildOutput.path.replace('/app', '');
      ...
      for (const dir of commonOutputDirs) {
        try {
          await DockerClient.executeCommand(`[ -d "${dir}" ] && echo "exists"`);
          finalBuildPath = dir;
      ...
      async function getAllFiles(dirPath: string): Promise<Record<string, string>> {
        ...
        const entries = DockerClient.executeCommand(`find ${dirPath} -type f`);

        for (const entry of entries) {
          const fullPath = path.join(dirPath, entry.name);

          if (entry.isFile()) {
            const content = DockerClient.executeCommand(`cat ${fullPath} | base64`);
