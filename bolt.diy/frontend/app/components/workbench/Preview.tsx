...
export const Preview = memo(({ setSelectedElement }: PreviewProps) => {
  ...
  const openInNewWindow = (size: WindowSize) => {
    if (activePreview?.baseUrl) {
      const match = activePreview.baseUrl.match(/^https?:\/\/([^.]+)\.local-credentialless\.dockerClient-api\.io/);

      if (match) {
        const previewId = match[1];
        const previewUrl = `/dockerClient/preview/${previewId}`;
        ...
        console.warn('[Preview] Invalid dockerClient URL:', activePreview.baseUrl);
      }
    }
  };
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          const match = activePreview.baseUrl.match(
                            /^https?:\/\/([^.]+)\.local-credentialless\.dockerClient-api\.io/,
                          );
