...
export function useGit(dockerClient: DockerClient) {
  ...
  useEffect(() => {
    ...
    fileData.current = {};
    setFs(getFs(dockerClient, fileData));
    setReady(true);
  }, [dockerClient]);

  const gitClone = useCallback(
    async (url: string, retryCount = 0) => {
      if (!dockerClient || !fs || !ready) {
        throw new Error('dockerClient not initialized. Please try again later.');
      }
      ...
      try {
        ...
        return { workdir: dockerClient.workdir, data };
      } catch (error) {
...
const getFs = (
  ...
  promises: {
    readFile: async (path: string, options: any) => {
      ...
      const relativePath = pathUtils.relative(dockerClient.workdir, path);

      try {
        const result = await dockerClient.readFile(relativePath, encoding);
    ...
    writeFile: async (path: string, data: any, options: any = {}) => {
      ...
      try {
        ...
        if (data instanceof Uint8Array) {
          ...
          const result = await dockerClient.writeFile(relativePath, data);
          return result;
        } else {
          // For text data, use the encoding if provided
          const encoding = options?.encoding || 'utf8';
          const result = await dockerClient.writeFile(relativePath, data, encoding);
    ...
    mkdir: async (path: string, options: any) => {
      ...
      try {
        const result = await dockerClient.mkdir(relativePath, { ...options, recursive: true });
    ...
    readdir: async (path: string, options: any) => {
      ...
      try {
        const result = await dockerClient.readDir(relativePath, options);
    ...
    rm: async (path: string, options: any) => {
      ...
      try {
        const result = await dockerClient.rm(relativePath, { ...(options || {}) });
    ...
    rmdir: async (path: string, options: any) => {
      ...
      try {
        const result = await dockerClient.rm(relativePath, { recursive: true, ...options });
    ...
    unlink: async (path: string) => {
      ...
      try {
        return await dockerClient.rm(relativePath, { recursive: false });
      } catch (error) {
        throw error;
      }
    },
    stat: async (path: string) => {
      try {
        ...
        const resp = await dockerClient.readDir(dirPath, { withFileTypes: true });
    ...
    lstat: async (path: string) => {
      return await getFs(dockerClient, record).promises.stat(path);
    },
    ...
    symlink: async (target: string, path: string) => {
      /*
       * Since Docker doesn't support symlinks,
    ...
    chmod: async (_path: string, _mode: number) => {
      /*
       * Container doesn't support changing permissions,
