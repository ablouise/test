...
export class WorkbenchStore {
  ...
  constructor(projectId?: string) {
    // Initialize project ID (from parameter, URL, or generate new)
    this.#projectId = projectId || this.#resolveProjectId();

    // Initialize Docker client with project context
    this.#dockerClient = new DockerClient();

    // Initialize stores with proper dependency order
    this.#filesStore = new FilesStore(this.#dockerClient, this.#projectId);
    this.#previewsStore = new PreviewsStore(this.#dockerClient);
    this.#editorStore = new EditorStore(this.#filesStore);
    this.#terminalStore = new TerminalStore(this.#dockerClient);

    // Persist project context
    this.#persistProjectContext();

    // HMR handling (existing code)
    if (import.meta.hot) {
      ...
      // Preserve binary files across HMR
  ...
  // Private method to resolve project ID from various sources
  #resolveProjectId(): string {
    // 1. Safe window access (only in browser)
    if (typeof window !== 'undefined') {
      const urlMatch = window.location.pathname.match(/\/projects\/([^\/]+)/);
      if (urlMatch?.[1]) return urlMatch[1];
    }

    // 2. Check local storage (also needs browser check)
    ...
    // 3. Generate new ID
    return `project-${Date.now().toString(36)}-${Math.random().toString(36).substring(2, 8)}`;
  }

  #persistProjectContext(): void {
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem('currentProjectId', this.#projectId);
    }

    if (typeof window !== 'undefined' && !window.location.pathname.includes(this.#projectId)) {
      const newPath = `/projects/${this.#projectId}`;
  ...
  async initializeProject(): Promise<void> {
    try {
      // Ensure container is ready before any operations
      await this.#dockerClient.ensureContainerReady(this.#projectId);

      // Initialize file system watchers
      await this.#filesStore.init();

      // Start any necessary services
      await this.#terminalStore.initialize();
  ...
  async switchProject(newProjectId: string): Promise<void> {
    ...
    await this.#dockerClient.stopContainer();
    this.#terminalStore.cleanup();
  ...
  async downloadZip() {
    const zip = new JSZip();
    const files = this.files.get();
    const projectName = (description.get() ?? 'project').toLocaleLowerCase().split(' ').join('_');
