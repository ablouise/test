...
export class PreviewsStore {
  ...
  async #init() {
    ...
    const dockerClient = this.#dockerClient;

    // Listen for Docker port events instead of dockerClient
    dockerClient.on('port', (event: { port: number; url: string }) => {
      this.#availablePreviews.set(event.port, {
        port: event.port,
        ready: true,
        baseUrl: event.url,
      });
      ...
      console.log('[Preview] Server ready on port:', event.port, event.url);
      this.broadcastUpdate(event.url);
      this._broadcastStorageSync();
    });

    // Start monitoring ports when container is ready
    if (dockerClient.currentContainerId) {
      await dockerClient.startPortMonitoring(dockerClient.currentContainerId);
    }
    ...
    dockerClient.on('port', (port, type, url) => {
...
export function usePreviewStore(dockerClient: DockerClient): PreviewsStore {
  // Initialize only once and reuse the same instance
  if (!previewsStore) {
    previewsStore = new PreviewsStore(dockerClient);
  }
