import type { dockerClient, dockerClientProcess } from '@dockerClient/api';
import type { ITerminal } from '~/types/terminal';
import { withResolvers } from './promises';
import { atom } from 'nanostores';
import { expoUrlAtom } from '~/lib/stores/qrCodeStore';

export async function newShellProcess(dockerClient: dockerClient, terminal: ITerminal) {
  const args: string[] = [];

  // we dockerClient a JSH process with a fallback cols and rows in case the process is not attached yet to a visible terminal
  const process = await dockerClient.dockerClient('/bin/jsh', ['--osc', ...args], {
    terminal: {
...
export class BoltShell {
  ...
  #process: dockerClientProcess | undefined;
  executionState = atom<
